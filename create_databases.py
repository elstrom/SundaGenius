...
def main():
    ...
    if st.session_state.nav == "Riwayat":
        ...
        user_data = pd.read_sql_query("SELECT * FROM users WHERE id = ?", conn, params=(user_id,))
        
        if not user_data.empty:
            username = st.text_input("Username", user_data['username'][0])
            email = st.text_input("Email", user_data['email'][0])
            phone = st.text_input("Nomor Telepon", user_data['phone'][0])
            password = st.text_input("Password", user_data['password'][0], type="password")
